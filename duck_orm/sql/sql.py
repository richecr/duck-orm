...
class QueryExecutor:
    ...
    @classmethod
    def select_sql(cls, name_table: str, fields: List[str], conditions: str, limit: int | None = None) -> str:
    ...
    def alter_table_add_column_with_constraint(
        ...
        return (
            ADD_COLUMN_FK_WITH_CONSTRAINT_SQL.format(**args, name_constraint=name_constraint)
            if name_constraint
    ...
    def add_foreing_key_column(
        ...
        return (
            ADD_FK_WITH_CONSTRAINT_WITHOUT_ALTER_SQL.format(**args, name_constraint=name_constraint)
            if name_constraint
    ...
    @classmethod
    def parser(cls, row: Mapping, fields: List[str] = [], fields_foreign_key: dict = {}) -> dict:
        entity = {}
        if not fields:
            ...
            for field in fields:
                ...
                try:
                    if row[field] is not None:
                        entity[field] = (
                            fields_foreign_key.get(field) if fields_foreign_key.__contains__(field) else row[field]
                        )
                except KeyError:
                    continue
