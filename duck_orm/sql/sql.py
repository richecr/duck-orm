...
SELECT_LAST_ID_ORDER_BY_SQL = (
    "SELECT {name_id} FROM {table} " + ORDER_BY_SQL + " LIMIT 1;"
)
...
ADD_FK_SQL = (
    "REFERENCES {table_relation} ({field}) ON DELETE {on_delete} ON UPDATE {on_update}"
)
ADD_FK_COLUMN_SQL = "FOREIGN KEY ({name}) " + ADD_FK_SQL
ADD_FK_WITH_CONSTRAINT_WITHOUT_ALTER_SQL = (
    "CONSTRAINT {name_constraint} " + ADD_FK_COLUMN_SQL
)
...
class QueryExecutor:
    ...
    @classmethod
    def insert_sql(
        cls, name_table: str, fields_name: List[str], placeholders: List[str]
    ):
    ...
    @classmethod
    def update_sql(
        cls, name_table: str, fields_values: List[str], conditions: List[str]
    ):
    ...
    @classmethod
    def select_sql(
        cls, name_table: str, fields: List[str], conditions: str, limit: int = None
    ) -> str:
