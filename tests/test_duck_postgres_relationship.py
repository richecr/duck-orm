...
from duck_orm.sql.relationship import ForeignKey, ManyToMany, OneToOne, OneToMany
...
class City(Model):
    ...
    def relationships(cls):
        cls.persons = OneToMany(model=Person, name_in_table_fk="city")
...
class Contact(Model):
    ...
    @classmethod
    def relationships(cls):
        cls.id_person: Person = OneToOne(model=Person, name_constraint="person_contact")
...
class User(Model):
    ...
    @classmethod
    def relationships(cls):
        cls.working_day = ManyToMany(model=WorkingDay, model_relation=UsersWorkingDay)
...
class UsersWorkingDay(Model):
    ...
    def relationships(cls):
        cls.users: User = ForeignKey(
            model=User, name_in_table_fk="id", name_constraint="users_working_days"
        )
...
def test_name_constraint_fk():
    ...
    assert (
        sql
        == "CREATE TABLE IF NOT EXISTS users_working_days (" + "id SERIAL PRIMARY KEY);"
    )


def get_table(table, tables):
    return any((tup["tablename"] == table) for tup in tables)
...
async def test_save_person():
    ...
    person_2 = Person(first_name="Elton", last_name="Ramalho", age=22, salary=1250)
    person_3 = Person(first_name="Naruto", last_name="Uzumaki", age=16, salary=500000)
    person_4 = Person(first_name="Hinata", last_name="Hyuga", age=16, salary=500000)
